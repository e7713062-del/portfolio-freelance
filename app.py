...
# ==============================================================================
# 5. Concrete Implementations & Infrastructure
# ==============================================================================
...
class SimpleTokenProvider(ITokenProvider):
    def generate_access_token(self, user: User) -> str:
        return f"access_token_{user.id}"

    def generate_refresh_token(self, user: User) -> str:
        return f"refresh_token_{user.id}"

    def decode_token(self, token: str) -> Dict[str, Any]:
        if token.startswith("access_token_"):
            user_id = token.replace("access_token_", "")
            return {"sub": user_id, "type": "access"}
        elif token.startswith("refresh_token_"):
            user_id = token.replace("refresh_token_", "")
            return {"sub": user_id, "type": "refresh"}
...
class InMemoryResourceRepository(AbstractResourceRepository):
    """تطبيق عملي للـ Repository باستخدام الذاكرة"""
...
class DatabaseChecker(BaseChecker):
    ...
    async def check(self) -> bool:
        if not self.db_session:
            return True  # Mocked as true when no DB session provided
        try:
...
# ==============================================================================
# 7. Dependency Injection Container Wiring
# ==============================================================================

# Singletons / Shared Repositories
global_user_repository = InMemoryUserRepository()
global_token_provider = SimpleTokenProvider()
global_password_hasher = SimplePasswordHasher()
global_resource_repository = InMemoryResourceRepository()
global_integration_service = IntegrationService()

def get_token_provider() -> ITokenProvider:
    return global_token_provider

def get_auth_service() -> AuthService:
    return AuthService(
        user_repo=global_user_repository,
        hasher=global_password_hasher,
        token_provider=global_token_provider
    )

def get_user_service() -> UserService:
    return UserService(user_repo=global_user_repository)

def get_resource_service() -> ResourceService:
    return ResourceService(repository=global_resource_repository)

async def get_db_session() -> Optional[AsyncSession]:
    return None

async def get_health_service(db: Optional[AsyncSession] = Depends(get_db_session)) -> HealthService:
...
# Auth & User Router
auth_user_router = APIRouter(prefix="/api/v1")

@auth_user_router.post("/auth/register", status_code=status.HTTP_201_CREATED, response_model=UserProfileDTO)
def register(
    dto: RegisterDTO, 
...
@auth_user_router.post("/auth/login", response_model=TokenResponseDTO)
def login(
    dto: LoginDTO, 
    auth_service: AuthService = Depends(get_auth_service)
):
...
@auth_user_router.post("/auth/refresh-token", response_model=TokenResponseDTO)
def refresh_token(
    refresh_token: str, 
    auth_service: AuthService = Depends(get_auth_service)
):
...
@auth_user_router.get("/users/me", response_model=UserProfileDTO)
...
@auth_user_router.put("/users/me", response_model=UserProfileDTO)
...
async def list_integrations():
    ...
    return await global_integration_service.list_available_integrations()

@integrations_router.post("/{provider}/connect", response_model=ConnectionResponse)
async def connect_integration(provider: str, request: ConnectRequest):
    """ربط خدمة خارجية جديدة عبر OAuth2 أو API Key"""
    return await global_integration_service.connect_service(provider, request)

@integrations_router.delete("/{integration_id}", status_code=status.HTTP_204_NO_CONTENT)
async def disconnect_integration(integration_id: str):
    """إلغاء ربط خدمة خارجية"""
    await global_integration_service.disconnect_service(integration_id)

@integrations_router.post("/webhooks/incoming/{provider}")
async def handle_incoming_webhook(
    ...
    success = await global_integration_service.process_incoming_webhook(
...
# Resource Router
resource_router = APIRouter(prefix="/api/v1/resources", tags=["System & Data Management"])

@resource_router.get("", response_model=dict, status_code=status.HTTP_200_OK)
def get_resources(
...
@resource_router.post("", response_model=ResourceResponseSchema, status_code=status.HTTP_201_CREATED)
def create_resource(
...
@resource_router.get("/{resource_id}", response_model=ResourceResponseSchema, status_code=status.HTTP_200_OK)
...
@resource_router.put("/{resource_id}", response_model=ResourceResponseSchema, status_code=status.HTTP_200_OK)
def update_resource(
    resource_id: int,
...
@resource_router.patch("/{resource_id}", response_model=ResourceResponseSchema, status_code=status.HTTP_200_OK)
def patch_resource(
    resource_id: int,
...
# Health Router
health_router = APIRouter(prefix="/api/v1/health", tags=["Health & Monitoring"])

@health_router.get("", status_code=status.HTTP_200_OK, summary="Liveness Probe")
